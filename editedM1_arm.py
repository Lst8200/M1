...
# #for cell 3, times it ecieves stim
# with open('spk_input.pkl', 'rb') as fileObj3: spk_input = pickle.load(fileObj3)

#input current
with open('/Users/latavia/netpyne/SLNC/M1 model/input_current.pkl', 'rb') as fileObj4: input_current= pickle.load(fileObj4)
input_current=np.array(input_current)

#sort spike times by cell number
spk_PT=np.sort(spk_pT, axis=0)
unique_id, indice_to_splt = np.unique(spk_PT[:,0],return_index=True)

#spkt = spike time = N x T rather thsn 1xT
spkt=np.split(spk_PT[:,1],indice_to_splt[1:]) #when individual cells spike, len = Num Cells
...
Theta2=[]
NetFiringFreq=[]
Unique_spkt=np.zeros((len(spkt),timewindows))

# #place unique spiketimes into matrix NumCells x Time
# for i in range(0,len(spkt)):
#     for j in range(0,timewindows):
#         if spkt[i][(spkt[i] >= (j*1)) & (spkt[i] < (j*1+10))].any():
#             Spkt=np.array(spkt[i])
#            #Unique_spkt[i][j]=j*((timewindows+1)/100) # u = j*1 or j*10
#             Unique_spkt[i][j]=Spkt[i]
#         else:
#             Unique_spkt[i][j] = 0
...
class ArmModel: #create arm movement
    ...
    def move_arm():   #calc freq -> number spikes/timewindow
        global Theta1,Theta2,xloc,yloc,joint1,joint2,timewindows,spk_PT,spk_pT

        for t in range(0,timewindows-1):  #time from 0 to 1000. this way, can take 1000 slices of time... [0,1], [1,2] .... [999,1000]
            j=t         

            #spikeTimes=spkt[i][(spkt[i] >= (j*10)) & (spkt[i]< (j*10+10))] 
            
            spikeTimes=spk_pT[:,1][(spk_pT[:,1] >= (j)) & (spk_pT[:,1]< (j+1))] #1ms window 
            #spikeTimes=spk_pT[:,1][(spk_pT[:,1] >= (j*10)) & (spk_pT[:,1]< (j*10+10))] #10ms window
...
# plt.figure()
# plt.plot(current_time,input_current, label='current')
# plt.xlabel('time')
...
# #theta vs stimulus
# plt.figure()
# plt.plot(timewindow, Theta1, label=r'$\theta_1$')
# plt.plot(timewindow, Theta2, label=r'$\theta_2$')
# plt.plot(current_time,input_current, label='current')
# plt.xlabel('time')
# plt.ylabel(r'$\theta$')
# plt.title(r'$\theta \/$ vs  stimulus')
# plt.legend()
# plt.show()
